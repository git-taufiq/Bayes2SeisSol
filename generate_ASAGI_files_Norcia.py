...
# load from .dat file
forwardmodel = np.loadtxt('Data/bestfit_Norcia.dat')
forwardmodel = forwardmodel[2:] # first two are dummies

# using 13 (along strike) x 7 (along dip) sparse grid
T0I = forwardmodel[0:91]
fsI = forwardmodel[91:182]
DcI = forwardmodel[182:273]

T0I_grid = np.zeros((13,7))
fsI_grid = np.zeros((13,7))
DcI_grid = np.zeros((13,7))
for i in range(13):
...
x = np.linspace(0, 24000, 13)
z = np.linspace(0, 12000, 7)
kind = 'linear'
...
grid = 100
xx = np.arange(0, 24000+grid, grid)
zz = np.arange(0, 12000+grid, grid) # using fault top depth at 0 m
...
fdI = 0.05 # mu_d in SeisSol (dynamic fric. coeff.)
cohesion = 0 # cohesion in SeisSol
T0_l = T0I_grid_new * 0
...
fout = Dataset('Norcia_fault_DR_2d_TnTsTd.nc','w',format='NETCDF4')
...
fout = Dataset('Norcia_fault_DR_2d_MusMudDcCoh.nc','w',format='NETCDF4')
fout.createDimension('x',nx)
fout.createDimension('z',nz)
fout.createVariable('x','f4',('x',))
fout.createVariable('z','f4',('z',))
